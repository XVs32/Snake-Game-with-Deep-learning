...
def generate_training_data(display, clock):
    ...
    for _ in tqdm(range(training_games)):
        ...
        for _ in range(step_per_game):
            ...
            direction_b, button_direction_b, tem_training_data_y_b = generate_training_data_y(snake_position_b,
...
def generate_training_data_y(snake_position, button_direction, direction, training_data_y,
                             front_blocked_distance, left_blocked_distance, right_blocked_distance):
    if direction == -1:
        if left_blocked_distance < 2:
            if front_blocked_distance < 5 and right_blocked_distance > 2:
                direction, button_direction = direction_vector(snake_position, 1)
                training_data_y.append([0, 0, 1])
            elif front_blocked_distance > 2 and right_blocked_distance < 2 :
                direction, button_direction = direction_vector(snake_position, 0)
                training_data_y.append([0, 1, 0])
            else:
                if right_blocked_distance > left_blocked_distance and right_blocked_distance > front_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, 1)
                    training_data_y.append([0, 0, 1])
                elif front_blocked_distance > left_blocked_distance and front_blocked_distance > right_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, 0)
                    training_data_y.append([0, 1, 0])
                else: 
        ...
        if front_blocked_distance < 2:
            if left_blocked_distance < 2 and right_blocked_distance > 2:
                direction, button_direction = direction_vector(snake_position, 1)
                training_data_y.append([0, 0, 1])
            elif left_blocked_distance > 2 and right_blocked_distance < 2 :
                direction, button_direction = direction_vector(snake_position, -1)
                training_data_y.append([1, 0, 0])
            else:
                if right_blocked_distance > left_blocked_distance and right_blocked_distance > front_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, 1)
                    training_data_y.append([0, 0, 1])
                elif left_blocked_distance > front_blocked_distance and left_blocked_distance > right_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, -1)
                    training_data_y.append([1, 0, 0])
                else: 
                    training_data_y.append([0, 1, 0])
        ...
        if right_blocked_distance < 2:
            if left_blocked_distance < 2 and front_blocked_distance > 2:
                direction, button_direction = direction_vector(snake_position, 0)
                training_data_y.append([0, 1, 0])
            elif left_blocked_distance > 2 and front_blocked_distance < 2 :
                direction, button_direction = direction_vector(snake_position, -1)
                training_data_y.append([1, 0, 0])
            else:
                if front_blocked_distance > left_blocked_distance and front_blocked_distance > right_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, 0)
                    training_data_y.append([0, 1, 0])
                elif left_blocked_distance > front_blocked_distance and left_blocked_distance > right_blocked_distance:
                    direction, button_direction = direction_vector(snake_position, -1)
